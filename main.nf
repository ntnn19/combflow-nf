...
workflow {
    ...
    chunks_ch = pdb_dir_per_comb_ch.collate(chunk_size).map { chunk ->
        def chunk_id = chunk[0][0]
        tuple(chunk_id, chunk.collect { t -> t[1] }, chunk.collect { t -> t[2] })
    }

    chunks_ch.take(1).view { "chunks_ch: $it" }

    COMBFOLD_ASSEMBLY(chunks_ch)
